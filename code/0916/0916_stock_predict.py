...
# LSTM model
def LSTM_model(drop=0.5, optimizer = 'adam', node=50):
    inputs = Input(shape= (25,5), name = 'input')
    x = LSTM(256, activation = 'relu', name = 'hidden1', return_sequences=True)(inputs)
    x = Dropout(drop)(x)
...
samsung = pd.read_csv('samsung0916.csv')
samsung = samsung[::-1]
...
#!jupyter nbconvert --to script 0916_stock_predict.ipynb
